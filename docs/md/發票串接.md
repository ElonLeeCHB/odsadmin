...
**位置**: `app\Domains\ApiPosV2\Routes\apipos.php`
...
**狀態說明**：目前這些是本系統對應前端的 API，**尚未串接廠商發票平台**。
...
```
app\Domains\ApiPosV2\Http\Controllers\Sale\
├── InvoiceController.php         # 發票 CRUD
├── InvoiceGivemeController.php   # Giveme API 串接
└── InvoiceTestController.php     # 測試用 Controller
```

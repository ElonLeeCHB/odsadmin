    ...
    public function updateOrCreate($data)
    {
        ...
        try {
            ...
            if(!empty($data['products'])){
                ...
                $this->ReceivingProductRepository->deleteByReceivingOrderId($receiving_order->id);
                ...
                foreach ($data['products'] as $key => $fm_receiving_product) {
                    if(empty($fm_receiving_product['product_id']) || !is_numeric($fm_receiving_product['product_id'])){
                ...
                //依照當前剛才整理後的陣列順序，重新設定排序欄位 sort_order 
                $sort_order = 1;
                foreach ($data['products'] as $key => $fm_receiving_product) {
                ...
                foreach ($data['products'] as $key => $fm_receiving_product) {
                    $product_id = $fm_receiving_product['product_id'];

                    if(empty($fm_receiving_product['receiving_unit_code'])){
                        $fm_receiving_product['receiving_unit_code'] = 'bag';
                    }
                    $row = [
                        'id' => $fm_receiving_product['receiving_product_id'] ?? null,
                        'receiving_order_id' => $receiving_order->id,
                        'product_id' => $fm_receiving_product['product_id'],
                        'product_name' => $fm_receiving_product['product_name'],
                        'product_specification' => $fm_receiving_product['product_specification'] ?? '',

                        'receiving_unit_code' => $fm_receiving_product['receiving_unit_code'] ?? '',
                        'receiving_unit_name' => $fm_receiving_product['receiving_unit_name'] ?? '',
                ...
                if(!empty($update_receiving_products)){
                    $this->ReceivingProductRepository->newModel()->upsert($update_receiving_products,['id']);

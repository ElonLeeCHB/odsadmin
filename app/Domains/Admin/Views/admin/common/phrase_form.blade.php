    ...
    <div class="page-header">
      <div class="container-fluid">
        <div class="float-end">
          ...
          {{-- <button type="submit" form="form-category" data-bs-toggle="tooltip" title="Save" class="btn btn-primary"><i class="fas fa-save"></i></button> --}}
              ...
              <div class="tab-content">
                <div id="tab-general" class="tab-pane active" >
                  ...
                  <div class="tab-content">
                    ...
                    <div id="language-{{ $language->code }}" class="tab-pane @if ($loop->first)active @endif">
                      <input type="hidden" name="translations[{{ $language->code }}][id]" value="{{ $phrase_translations[$language->code]['id'] ?? '' }}" >

                      <div class="row mb-3 required">
                        <label for="input-name-{{ $language->code }}" class="col-sm-2 col-form-label">{{ $lang->column_name }}</label>
                        <div class="col-sm-10">
                          <div class="input-group">
                            <input type="text" name="translations[{{ $language->code }}][name]" value="{{ $phrase_translations[$language->code]['name'] ?? ''  }}" placeholder="{{ $lang->column_name }}" id="input-name-{{ $language->code }}" class="form-control">
                                                    </div>
                      ...
                      <div class="row mb-3 ">
                        ...
                        <div class="col-sm-10">
                          <div class="input-group">
                            <input type="text" name="translations[{{ $language->code }}][short_name]" value="{{ $phrase_translations[$language->code]['short_name'] ?? ''  }}" placeholder="{{ $lang->column_short_name }}" id="input-short_name-{{ $language->code }}" class="form-control">
                                                    </div>
                ...
                <div id="tab-data" class="tab-pane">
                      ...
                      <div class="input-group">
                        <select name="taxonomy_code" id="input-taxonomy_code" class="form-select">
                          <option value="0">{{ $lang->text_none }}</option>
                            <option value="phrase_order_comment"@if($phrase->taxonomy_code == 'phrase_order_comment') selected @endif>訂單客戶備註 (phrase_order_comment)</option>
                            <option value="phrase_order_extra_comment"@if($phrase->taxonomy_code == 'phrase_order_extra_comment') selected @endif>訂單餐點備註 (phrase_order_extra_comment)</option>
                        </select>
                      </div>
                    </div>
                  </div>
                  <div class="row mb-3">
                    ...
                    <div class="col-sm-10">
                      <input type="text" name="sort_order" value="{{ $phrase->sort_order }}" placeholder="Sort Order" id="input-sort_order" class="form-control">
                    </div>
                  </div>
                    ...
                    <div class="col-sm-10">
                      <div class="input-group">
                        <div id="input-is_active" class="form-check form-switch form-switch-lg">
                          ...
                          <input type="checkbox" name="is_active" value="1" class="form-check-input" @if($phrase->is_active) checked @endif/>
                        </div>
                      </div>
                    </div>
                  </div>
                ...
                <input type="hidden" name="phrase_id" value="{{ $phrase_id }}" id="input-phrase_id"/>
              </div>
            </form>
            </div>
            </div>

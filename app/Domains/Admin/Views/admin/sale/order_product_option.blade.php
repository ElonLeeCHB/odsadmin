    ...
    <tr class="tr_drink">
      ...
      @for($i = 0; $i < 10; $i++)
        ...
        @foreach($product_options['drink']['product_option_values'] as $product_option_value)
          ...
          <input type="text"   name="order_products[{{ $product_row }}][order_product_options][{{ $poid }}][product_option_values][{{ $povid }}][parent_povid][{{ $parent_povid }}][quantity]" value="{{ $quantity }}" style="width:38px;" class="text-start drink"><BR>

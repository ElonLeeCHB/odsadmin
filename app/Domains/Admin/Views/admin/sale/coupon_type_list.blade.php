    ...
    <table class="table table-bordered table-hover">
      ...
      <tbody>
        @foreach($couponTypes as $couponType)
				<tr>
          <td class="text-center"><input type="checkbox" name="selected[]" value="{{ $couponType->id }}" class="form-check-input"/></td>
          <td class="text-start">{{ $couponType->id }}</td>
          <td class="text-start">{{ $couponType->name }}</td>
          <td class="text-start">{{ $couponType->is_active }}</td>
          <td class="text-end"><a href="{{ $couponType->edit_url }}" data-bs-toggle="tooltip" title="Edit" class="btn btn-primary"><i class="fa-solid fa-pencil"></i></a></td>
				</tr>
        @endforeach
			</tbody>
    </table>

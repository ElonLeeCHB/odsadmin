    ...
    <div class="container-fluid">
      <div class="card">
        <div class="card-body">
          ...
          <form id="form-requisition" method="post" data-oc-toggle="ajax">
            @csrf
            @method('POST')
...
$("#btn-redirectToRequiredDate").on('click', function(){ 
  ...
  if(required_date_2ymd.length > 0){
    window.location.href = "{{ route('lang.admin.sale.orderingredientsdailies.form') }}/" + required_date_2ymd;
  }
});
...
function calcOrders(){
  ...
  $.ajax({
    type:'get',
    //dataType: 'json',
    url: "{{ route('lang.admin.sale.orderingredientsdailies.calcMrequisitionsByDate') }}/"+required_date,
...
$(function(){
  ...
  $('#href-printForm').on('click',function(e){
    ...
    var url = "{{ $print_form_url }}/" + required_date;
    window.open(url);
  });
})

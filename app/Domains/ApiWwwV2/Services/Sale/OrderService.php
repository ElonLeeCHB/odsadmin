...
class OrderService extends Service
{
    ...
    public function saveCustomer($data)
    {
        ...
        if (!empty($customer_id)){
            $member = User::findOrFail($customer_id);
        }
        else if (!empty($mobile)) {
            $member = User::firstOrNew(['mobile' => $mobile]);
        } 
    ...
    public function addOrder($data)
    {
        try {
            ...
            foreach ($product_ids as $product_id) {
                ...
                $db_products[$db_product_id] = $db_product;
            }

            foreach ($db_products as $db_product_id => $db_product) {
                // $db_products[$db_product_id] = $db_product->toArray();
                $db_products[$db_product_id] = $db_product;
            }
